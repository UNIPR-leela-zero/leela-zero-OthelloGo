...
void SMP::Lock::lock() {
    ...
    // all cases
    // (Il thread continua a verificare se il mutex � disponibile finch� non � in grado di acquisirlo in modo sicuro)
...
void SMP::Lock::unlock() {
    // Verifica se il blocco attuale possiede gi� il lock.
    assert(m_owns_lock);
    ...
    assert(lock_held);
#endif
    // Il blocco non possiede pi� il mutex.
    m_owns_lock = false;
}
