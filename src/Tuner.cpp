...
Parameters Tuner<net_t>::get_parameters_by_int(
    ...
    // For each parameter count the possible choices and multiply them
    // to obtain the configurations.
    auto cfgs = 1;
...
std::string Tuner<net_t>::parameters_to_string(const Parameters& p) {
    ...
    // Eventually remove the extra space added at the end of the
    // string in the last for cycle.
    if (s.size() > 0) {
        s.resize(s.size() - 1);
    }
    return s;
...
// Compare the results of the multiplication between matrices with
// referenced results to evaluate the accuracy of the algorithm.
template <typename net_t>
...
std::string Tuner<net_t>::tune_sgemm(const int m, const int n, const int k,
    ...
    for (auto& p : valid_params) {
        ...
        // Calculate local and global sizes for the OpenCL kernel
        // execution based on the configuration parameters.
        ...
        for (auto r = 0; r < runs; r++) {
            try {
                ...
                // Calculates the error compared to the referenced
                // version and sum the execution times for all
                // iterations.
                auto this_error =
        ...
        // Error less than the maximum limit and previous best
        // execution time, updates the optinal parameters.
        if (error < getTunerMaxError<net_t>()
    ...
    // Handles the case in which no working configuration was found
...
std::string Tuner<net_t>::sgemm_tuners_from_line(std::string line, const int m,
    ...
    // Checks if the parameters m, n, k and batch_size in the line
    // correspond to the provided values (s[1] to s[5]).
    ...
    // Checks if the name of the OpenCL device in the line corresponds
    // to the current device name.
