...
# 16 planes, 1 side to move, 1 x 65 probs, 1 winner = 8 lines
...
class ChunkParser:
    def __init__(self, chunkdatasrc, shuffle_size=1, sample=1,
        ...
        """
            ...
            v1: The original text format describing a move. 8 lines long.
        ...
        self.prob_reflection_table = [
            [remap_vertex(vertex, sym)
              for vertex in range(64)]+[64] for sym in range(8)]
        # Build full 16-plane reflection tables.
        self.full_reflection_table = [
            np.array([remap_vertex(vertex, sym) + p * 64
                for p in range(16) for vertex in range(64)])
                    for sym in range(8)]
        ...
        self.flat_planes = [ b'\1'*64 + b'\0'*64, b'\0'*64 + b'\1'*64 ]
    ...
    def convert_v1_to_v2(self, text_item):
        ...
        # We flatten to a single array of len 16*8*8, type=np.uint8
        ...
        if np.any(np.isnan(probabilities)):
            ...
            return False, None
        if not(len(probabilities) == 65):
            return False, None

        probs = probabilities.tobytes()
        if not(len(probs) == 65 * 4):
            return False, None
    ...
    def convert_v2_to_tuple(self, content):
        ...
        assert len(planes) == (18 * 8 * 8), len(planes)
...
class ChunkParserTest(unittest.TestCase):
    def generate_fake_pos(self):
        """
            ...
            Result is ([[64] * 18], [65], [1])
        """
        # 1. 18 binary planes of length 64
        planes = [np.random.randint(2, size=64).tolist()
                  for plane in range(16)]
        stm = float(np.random.randint(2))
        planes.append([stm] * 64)
        planes.append([1. - stm] * 64)
        # 2. 65 probs
        probs = np.random.randint(3, size=65).tolist()
    ...
    def test_parsing(self):
        ...
        for p in range(16):
            ...
            h = np.packbits([int(x) for x in planes[p][0:64]]).tobytes().hex()
        # then side to move
            ...
            for symmetry in range(8):
                ...
                sym_probs = [
                    probs[remap_vertex(vertex, symmetry)]
                        for vertex in range(64)] + [probs[64]]
